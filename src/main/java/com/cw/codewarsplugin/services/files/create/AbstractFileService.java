...
import com.cw.codewarsplugin.exceptions.SourcesRootNotFoundException;
import com.cw.codewarsplugin.models.kata.JsonSource;
...
public abstract class AbstractFileService {
    ...
    public void throwModuleNotFoundException(String language) {
        throw new ModuleNotFoundException(MessageFormat.format(StringConstants.MODULE_NOT_FOUND, language));
    }
    ...
    public Module pickModule() {
        if (modules.size() == 0){
            ...
            ApplicationManager.getApplication().invokeAndWait(() -> {
                index.set(Messages.showIdeaMessageDialog(project, String.format(StringConstants.SEVERAL_MODULES, input.getLanguageName()), String.format(StringConstants.PICK_MODULE, input.getLanguageName()), modules.stream().map(Module::getName).toArray(String[]::new), 0, IconLoader.getIcon(StringConstants.MESSAGE_ICON, SidePanel.class), null));
            });
    ...
    public void getSourcesRoot() {
        ...
        if (sourcesRoots.size() == 1) {
            ...
            AtomicInteger index = new AtomicInteger(0);
            ApplicationManager.getApplication().invokeAndWait(() -> {
                index.set(Messages.showIdeaMessageDialog(project, StringConstants.SEVERAL_SOURCES, StringConstants.PICK_SOURCE, sourcesRoots.stream().map(VirtualFile::getName).toArray(String[]::new), 0, IconLoader.getIcon(StringConstants.MESSAGE_ICON, SidePanel.class), null));
            });
